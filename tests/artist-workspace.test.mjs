...
test('Social dashboard loads content only and makes no legacy CRM request',async()=>{const r=runtime({role:null,authUser:{id:UUID,app_metadata:{crm_enabled:true,crm_role:'social'}}});const out=await r.request({action:'dashboard'},{authorization:'Bearer test-jwt'});assert.equal(out.body.ok,true);assert.equal(r.outbound(),0);assert.deepEqual(r.calls.map(q=>q.table),['artist_content','content_publications','artist_metrics']);});
